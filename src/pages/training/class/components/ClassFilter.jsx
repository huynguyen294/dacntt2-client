      ...
      <PopoverContent>
        <div className="p-2 space-y-4">
          ...
          <RadioGroup value={status} label="Trạng thái" onValueChange={setStatus}>
            ...
            <Radio key="pending" value="pending">
              Chưa bắt đầu
            </Radio>
            <Radio key="active" value="active">
              Đang hoạt động
            </Radio>
            <Radio key="ended" value="ended">
              Đã kết thúc
            </Radio>
          </RadioGroup>
          <RadioGroup
            ...
            <Button
              onPress={() => {
                ...
                if (status !== "all") {
                  switch (status) {
                    case "pending":
                      newFilters.openingDay = { gte: format(new Date(), DATE_FORMAT) };
                      break;
                    case "active":
                      newFilters.openingDay = { lte: format(new Date(), DATE_FORMAT) };
                      newFilters.closingDay = { gte: format(new Date(), DATE_FORMAT) };
                      break;
                    case "ended":
                      newFilters.closingDay = { lte: format(new Date(), DATE_FORMAT) };
                      break;
                  }
                }
                changePager("page", 1);
